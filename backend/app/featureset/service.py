...
def profiling(item: FeatureSetProfiling):
    try:
        ...
        if datasource.origin == OriginType.MYSQL:
            data = extract_mysql_to_df(datasource.connection_detail, item.collection)
...
def _transform_featureset(
    ...
    try:
        ...
        used_df, feature_transformers, target_transformer = preprocess.build_transformer(
            df=data,
            features_metadata=item.features,
            target_metadata=item.target,
        )
        X_transformed = feature_transformers.fit_transform(used_df)
        ...
        if target_transformer:
            y_transformed = target_transformer.fit_transform(used_df[[target_name]])
